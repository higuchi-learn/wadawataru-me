  ...
  return (
    <div className={`flex flex-col gap-1 items-start p-1 ${className ?? ''}`}>
      <div className="border-b border-[var(--border)] h-7 relative overflow-hidden shrink-0 w-full">
        <p className="absolute top-0 left-0 text-xl font-bold leading-7 text-black whitespace-nowrap">{title}</p>
      </div>
      <p className="text-sm font-normal leading-5 text-black shrink-0 whitespace-nowrap">{description}</p>
    </div>
  );
}
