...
import { MOCK_ARTICLE as MOCK_ARTICLE_DATA } from "@/lib/mockArticle";
...
const MOCK_ARTICLE = {
  ...MOCK_ARTICLE_DATA,
  publishStatus: MOCK_ARTICLE_DATA.publishStatus as PublishStatus,
};

export default function BlogEditPage() {
  const [title, setTitle] = useState(MOCK_ARTICLE.title);
  const [description, setDescription] = useState(MOCK_ARTICLE.description);
  const [tags, setTags] = useState<string[]>(MOCK_ARTICLE.tags);
  const [thumbnail, setThumbnail] = useState(MOCK_ARTICLE.thumbnail);
  const [slug, setSlug] = useState(MOCK_ARTICLE.slug);
  const [content, setContent] = useState(MOCK_ARTICLE.content);
  const [publishStatus] = useState<PublishStatus>(MOCK_ARTICLE.publishStatus);
  const [savedAt] = useState(MOCK_ARTICLE.savedAt);
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 px-1">
      {/* ヘッダー: edit モードなので StatusTag + 保存日時 + アーカイブボタンを表示 */}
      <AdminHeader
        genre="blog"
        status="edit"
